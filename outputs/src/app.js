...
const renderBadges = () => `<section class="screen" aria-labelledby="badges-title"><div class="missions-head"><div><p class="eyebrow">Your collection</p><h1 id="badges-title">Badge shelf.</h1><p>Badges are reminders of what your hands and brain can do.</p></div><div class=…65 tokens truncated…dge(badge.level); return `<article class="card badge-page-card ${earned ? '' : 'locked'}"><div class="badge-page-icon">${badge.icon}</div><h3>${badge.name}</h3><p>${badge.description}</p>${earned ? '<span class="badge-status">Earned</span>' : '<span class="badge-status">Locked</span>'}</article>`; }).join('')}</div>${childNav('badges')}</section>`;
